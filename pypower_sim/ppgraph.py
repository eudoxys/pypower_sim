...
- The weighted incidence matrix is obtained using
  ...
  which `print(W.todense())` outputs as
        ...
        [[ 3.81562882,  5.16956162,  0.        ,  0.        ,  0.        ],
         [-3.81562882,  0.        ,  0.        ,  5.16956162,  0.        ],
         [ 0.        , -5.16956162,  0.        ,  0.        ,  3.02370585],
         [ 0.        ,  0.        ,  0.        , -5.16956162, -3.02370585]]
...
- The graph Laplacian matrix is obtained using
  ...
  which `print(G.todense())` outputs as
        ...
        [[ 8.98519044, -3.81562882, -5.16956162,  0.        ],
         [-3.81562882,  8.98519044,  0.        , -5.16956162],
         [-5.16956162,  0.        ,  8.19326748, -3.02370585],
         [ 0.        , -5.16956162, -3.02370585,  8.19326748]]

- The spectral analysis results are obtained using

        S = graph.spectral(weighted=True,complex_flows=False)

  which `print(f"{S.E=}",f"{S.U=}",f"{S.K=}",sep="\n")` outputs as

        S.E=array([ 0.        ,  6.77902924, 10.33912324, 17.23876334])
        S.U=array([[ 0.5       ,  0.5       ,  0.5       ,  0.5       ],
               [-0.46059115,  0.46059115, -0.53652194,  0.53652194],
               [-0.5       , -0.5       ,  0.5       ,  0.5       ],
...
- Only weighted results consider branch status using zero-admittance. Unweighted results
  will return all possible connections regardless of branch status.
"""
...
class PPGraph:
    ...
    def incidence(self,
        ...
        - `weighted`: generated weighted incidence based on square root of
          line admittance (inverse of impedance $R+jX$) for closed lines
